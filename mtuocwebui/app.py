...
from mtuoctranslatefiles import get_supported_formats
...
from mtuocwebui import flood, remove_translated_files, scheduler, secret, security, storage
from mtuocwebui.locales import (
    _,
    _lazy,
...
def get_upload_dir():
    upload_dir = os.path.join(tempfile.gettempdir(), "mtuocwebui-files-translate")
...
def create_app(args):
    ...
    if args.metrics:
      ...
      measure_request = Summary('mtuocwebui_http_request_duration_seconds', 'Time spent on request', ['endpoint', 'status', 'request_ip', 'api_key'])
      measure_request.labels('/translate', 200, '127.0.0.1', '')

      gauge_request = Gauge('mtuocwebui_http_requests_in_flight', 'Active requests', ['endpoint', 'request_ip', 'api_key'], multiprocess_mode='livesum')
    ...
    def translate_file():
        ...
        if os.path.splitext(file.filename)[1] not in frontend_mtuoc_supported_files_format:
        ...
        return jsonify(
            {
                ...
                "supportedFilesFormat": [] if args.disable_files_translation else frontend_mtuoc_supported_files_format,
